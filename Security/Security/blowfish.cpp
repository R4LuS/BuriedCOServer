...
//Blowfish S-Boxes
const uint32_t Blowfish::S[4][256] = {
    ...
    : mEnNum(0), mDeNum(0)
{
    ...
    memset(mEnIV, 0, Blowfish::BLOCK_SIZE);
    memset(mDeIV, 0, Blowfish::BLOCK_SIZE);
}
...
Blowfish :: generateKey(const uint8_t* aSeed, size_t aLen)
{
    ...
    {
        uint32_t data = 0;
        for (int32_t n = 4; n != 0; --n, ++x)
        {
            data = (data << 8) | aSeed[x % aLen];
        }
        mP[i] ^= data;
    }
    uint8_t block[Blowfish::BLOCK_SIZE];
    memset(block, 0, sizeof(block));
    for (int32_t i = 0, max = Blowfish::ROUNDS + 2; i < max; )
    {
        encipher(block);
        mP[i++] = ((uint32_t*)block)[0];
        mP[i++] = ((uint32_t*)block)[1];
    }
    for (int32_t j = 0; j < 4; ++j)
    {
        for (int32_t k = 0; k < 256; )
        {
            encipher(block);
            mS[j][k++] = ((uint32_t*)block)[0];
            mS[j][k++] = ((uint32_t*)block)[1];
        }
    }
}
...
void
Blowfish :: encrypt(uint8_t* aBuf, size_t aLen)
{
    ...
    for (size_t i = 0; i < aLen; ++i)
    {
        ...
        *(aBuf++) = val;
        ...
        mEnNum = (mEnNum + 1) % (Blowfish::BLOCK_SIZE);
    }
}

void
Blowfish :: decrypt(uint8_t* aBuf, size_t aLen)
{
    //Cipher feedback (CFB64) mode is used on the variable length buffer...
    ...
    for (size_t i = 0; i < aLen; ++i)
    {
        ...
        *(aBuf++) = val;
        ...
        mDeNum = (mDeNum + 1) % (Blowfish::BLOCK_SIZE);
    }
}
...
void
Blowfish :: encipher(uint8_t aBlock[Blowfish::BLOCK_SIZE])
{
    ...
    for (int32_t i = 0, max = Blowfish::ROUNDS; i < max; ++i)
    {
        lv ^= mP[i];
        rv ^= F(lv);
    ...
    lv ^= mP[Blowfish::ROUNDS];
    rv ^= mP[Blowfish::ROUNDS + 1];
...
void
Blowfish :: decipher(uint8_t aBlock[Blowfish::BLOCK_SIZE])
{
    ASSERT(aBlock != nullptr);
    ...
    for (int32_t i = Blowfish::ROUNDS + 1; i > 1; --i)
    {
        lv ^= mP[i];
        rv ^= F(lv);
